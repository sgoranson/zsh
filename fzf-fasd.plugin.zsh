...
__fzf_fasd_zsh_completion() {
  ...
  if [[ "$matches_count" -gt 1 ]]; then
    ...
    #     | FZF_DEFAULT_OPTS="--height 50% --reverse --bind 'shift-tab:up,tab:down'" fzf --query="$slug"
    # selected=$(__fzf_fasd_generate_matches "$slug" | FZF_DEFAULT_OPTS="--cycle --height 50% --reverse --bind 'shift-tab:up,tab:down'" fzf --query="$slug"
    selected=$(__fzf_fasd_generate_matches "$slug" | FZF_DEFAULT_OPTS="--exact --cycle --bind 'shift-tab:up,tab:down'" fzf --query="$slug"
    )
